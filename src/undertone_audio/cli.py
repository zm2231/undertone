...
_COMMAND_GROUPS: list[tuple[str, list[str]]] = [
    ...
    ("Speakers", ["fingerprints", "fingerprint-label"]),
    ("Maintenance", ["delete"]),
...
def _overview_text(subcommands: argparse._SubParsersAction) -> str:
    ...
    names = list(subcommands.choices)
    width = max((len(name) for name in names), default=0)

    grouped: set[str] = set()
    blocks: list[str] = ["Commands:"]
    for title, group in _COMMAND_GROUPS:
        present = [name for name in group if name in subcommands.choices]
        if not present:
            continue
        lines = [f"  {title}:"]
        for name in present:
